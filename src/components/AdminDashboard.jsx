...
const StatusChip = ({ status }) => {
  const statusConfig = {
    'En attente': { bg: '#fff3e0', text: '#ef6c00', icon: '⏳' },
    Effectuée: { bg: '#e8f5e9', text: '#2e7d32', icon: '✅' },
    'Dépôt confirmé': { bg: '#e3f2fd', text: '#1976d2', icon: '💳' },
    Expirée: { bg: '#ffebee', text: '#d32f2f', icon: '⌛' },
    Annulée: { bg: '#f5f5f5', text: '#616161', icon: '❌' },
  };
...
const TransactionTable = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      getActions: (params) =>
        [
          params.row.status === 'Dépôt confirmé' && (
            <GridActionsCellItem
              key="validate"
